...
class CocoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.geo_feats_folder is not None:
            #geo_feats = np.load(os.path.join(self.geo_feats_folder, str(image_id) + '_geo.npz'))['geo_features']
            #geo_feats = np.load(os.path.join(self.geo_feats_folder, str(image_id) + '_tran.npz'))['combined_features']
            #geo_feats = np.load(os.path.join(self.geo_feats_folder, str(image_id) + '_gtran.npz'))['transformed_features']
            #geo_feats = np.load(os.path.join(self.geo_feats_folder, str(image_id) + '_gten.npz'))['geometric_features']
            #geo_feats = np.load(os.path.join(self.geo_feats_folder, str(image_id) + '_gfifteen.npz'))['geometric_features']
            geo_feats = np.load(os.path.join(self.geo_feats_folder, str(image_id) + '_tranfifty.npz'))['transformed_features']
            geo_feats = np.array(geo_feats).astype('float32')
        else:
            geo_feats = np.zeros((1, 1))
