...
class LowRankBilinearEncBlock(nn.Module):
    def __init__(
        ...
        #self.geo_transform = nn.Linear(geo_dim, embed_dim)  # Linear transform for geo_feats
        ...
        self.dual_attention = GeoAttention(embed_dim)  # Dual attention
    ...
    def forward(self, gv_feat, att_feats, att_mask, geo_feats, p_att_feats=None):
        geo_feats = self.geo_embed(geo_feats)
        att_feats = self.dual_attention(att_feats, geo_feats)  # Apply geometric attention
        #att_feats = att_feats * geo_feats
        ...
        for i, layer in enumerate(self.layers):
            gv_feat = layer(gv_feat, att_feats, att_mask, gv_feat, att_feats, geo_feats)
...
class LowRankBilinearDecBlock(nn.Module):
    ...
    def forward(self, gv_feat, att_feats, att_mask, geo_feats, p_att_feats=None, precompute=False):
        ...
        for i, layer in enumerate(self.layers):
            ...
            gv_feat = layer(gv_feat, key, att_mask, gv_feat, value2, geo_feats, precompute)
            feat_arr.append(gv_feat)
